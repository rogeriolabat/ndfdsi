...
def column_to_list(data, index):
    ...
    for l in range(len(data)):
        column_list.append(data[l][index])
    return column_list
...
for m in range(1,len(data_list)) :
    if data_list[m][6]=="Male":
        male += 1
    if data_list[m][6]=="Female":
        female += 1
...
def count_gender(data_list):
    ...
    Função que conta a quantidade de cada gênero em uma lista.
      ...
      Retorna:
          Uma lista com as quantidades de homem e mulher, no formato [qtde_male, qtde_female].
    """
    male = 0
    female = 0
    for m in range(1,len(data_list)) :
...
def most_popular_gender(data_list):
    ...
    """
    f = lambda x,y: "Male" if x>y else "Equal" if x==y else "Female"
    answer = f(count_gender(data_list)[0],count_gender(data_list)[1])
    return answer
...
def count_user_types(data_list):
    ...
    Função que conta a quantidade de cada tipo de usuario em uma lista.
      Argumentos:
          data_list: A lista de tipos usuario (list).
      Retorna:
          Uma lista com as quantidades de Subscriber e Customer.
    """
    Subscriber = 0
    Customer = 0
    for m in range(1,len(data_list)) :
...
answer = "Porque existem Registros com a coluna genero em branco (sem preencher)"
...
min_trip = round(sorted(trip_duration_list)[0])
max_trip = round(sorted(trip_duration_list)[len(trip_duration_list)-1])

qtde_itens = len(trip_duration_list)

soma_total = 0
for t in range(len(trip_duration_list)):
    soma_total += int(trip_duration_list[t])
mean_trip=round(soma_total/qtde_itens)

meio = int(qtde_itens/2)
median_trip = int(sorted(trip_duration_list)[meio])
